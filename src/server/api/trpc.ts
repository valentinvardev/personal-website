...
const timingMiddleware = t.middleware(async ({ next, path }) => {
  ...
  const end = Date.now();
  if (t._config.isDev) console.log(`[TRPC] ${path} took ${end - start}ms to execute`);
 ...
 * Admin procedure
 *
 * Requiere la cookie de sesión del panel /admin (ver ~/server/admin-auth).
 */
